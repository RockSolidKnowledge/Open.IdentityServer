...
public class DefaultTelemetryService : ITelemetryService, IDisposable
{
        ...
        string error = null)
    {
        ...
        var tokenTypes = new (bool issued, string type)[]
        {
            (accessTokenIssued,  IdentityServerConstants.TokenTypes.AccessToken),
            (idTokenIssued,      IdentityServerConstants.TokenTypes.IdentityToken),
            (refreshTokenIssued, IdentityServerConstants.PersistedGrantTypes.RefreshToken),
        };

        tags.Add(TelemetryConstants.TagConstants.TokenType,
            tokenTypes.Where(t => t.issued)
                .Select(t => t.type)
    ...
    public ITrace Trace(string category, string activityName)
    {
        if (_activitySources.TryGetValue(category, out var activitySource))
        {
            ...
            return new TraceActivityAdapter(activity);
        }
    ...
    /// <inheritDoc/>
    public ITrace Trace(string category, object caller, [CallerMemberName] string callingMethod = null)
    {
    ...
    private class TraceActivityAdapter : ITrace
    {
        private Activity _activity;

        public TraceActivityAdapter(Activity activity)
        {
            _activity = activity;
        }
        ...
        public ITrace AddTag(string key, string value)
        {
            _activity?.AddTag(key, value);
            return this;
        }

        public ITrace AddTag(string key, object value)
        {
            _activity?.AddTag(key, value);
            return this;
        }

        public ITrace AddException(Exception exception)
        {
