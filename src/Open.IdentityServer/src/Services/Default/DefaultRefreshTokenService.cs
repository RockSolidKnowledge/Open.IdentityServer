...
public class DefaultRefreshTokenService : IRefreshTokenService
{
    ...
    public virtual async Task<TokenValidationResult> ValidateRefreshTokenAsync(string tokenHandle, Client client)
    {
        ...
        trace?.AddTag(TelemetryConstants.TagConstants.Client, client.ClientId);
    ...
    {
        using var trace = Telemetry.Trace(TelemetryConstants.TraceCategories.Services, this);
        trace?.AddTag(TelemetryConstants.TagConstants.Client, request.Client.ClientId);
        trace?.AddTag(TelemetryConstants.TagConstants.Subject, request.Subject.GetSubjectId());
        ...
        Client client)
    {
        using var trace = Telemetry.Trace(TelemetryConstants.TraceCategories.Services, this);
        trace?.AddTag(TelemetryConstants.TagConstants.Client, client.ClientId);
        if (refreshToken.ClientId != null) trace?.AddTag(TelemetryConstants.TagConstants.Subject, refreshToken.SubjectId);
