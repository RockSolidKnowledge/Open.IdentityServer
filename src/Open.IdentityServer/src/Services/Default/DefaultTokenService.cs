...
public class DefaultTokenService : ITokenService
{
    ...
    public virtual async Task<Token> CreateIdentityTokenAsync(TokenCreationRequest request)
    {
        ...
        trace
            ?.AddTag(TelemetryConstants.TagConstants.Client, request.ValidatedRequest.Client.ClientId)
            ?.AddTag(TelemetryConstants.TagConstants.Subject, request.Subject?.GetSubjectId());
    ...
    public virtual async Task<string> CreateSecurityTokenAsync(Token token)
    {
        using var trace = Telemetry.Trace(TelemetryConstants.TraceCategories.Services, this);
        trace
            ?.AddTag(TelemetryConstants.TagConstants.Client, token.ClientId)
            ?.AddTag(TelemetryConstants.TagConstants.Subject, token.SubjectId);
        string tokenResult;
