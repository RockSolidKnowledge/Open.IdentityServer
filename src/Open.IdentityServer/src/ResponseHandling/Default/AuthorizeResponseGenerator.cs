...
public class AuthorizeResponseGenerator : IAuthorizeResponseGenerator
{
    ...
    {
        using var trace = TelemetryService.Trace(TelemetryConstants.TraceCategories.Basic, this);
        trace
            ?.AddTag(TelemetryConstants.TagConstants.Client, request.Client.ClientId)
            ?.AddTag(TelemetryConstants.TagConstants.GrantType, request.GrantType);
