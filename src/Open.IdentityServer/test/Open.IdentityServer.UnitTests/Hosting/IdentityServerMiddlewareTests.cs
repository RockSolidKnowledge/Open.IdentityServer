...
public class IdentityServerMiddlewareTests
{
    ...
    [Fact]
    public async Task Invoke_WhenRouterLocatesEndpoint_ShouldTrace()
    {
        var endpointMock = new Mock<IEndpointHandler>();
        var endpointResult = new Mock<IEndpointResult>();
        ...
        _telemetryService.Verify(x => x.Trace(It.IsAny<string>(), It.IsAny<string>()), Times.Once);
        _trace.Verify(x => x.Dispose(), Times.Once);
    }

    [Fact]
    public async Task Invoke_WhenRouterEndpointIsNull_ShouldNotTrace()
    {
        _router.Setup(x => x.Find(_context)).Returns((IEndpointHandler)null);
        
        await InvokeSubjectMiddleware();
        
        _telemetryService.Verify(x => x.Trace(It.IsAny<string>(), It.IsAny<string>()), Times.Never);
    }
    ...
    [Fact]
    public async Task Invoke_OnUnhandledException_ShouldAttachToTrace()
    {
        ...
        _trace.Verify(t => t.AddException(exception), Times.Once);
    }
