...
public class DefaultTelemetryServiceTests : IDisposable
{
    ...
    [Theory]
    [InlineData("clientid", "grant", true, true, true, null)]
    [InlineData("clientid", "grant", false, false, false, "some error")]
        ...
        string error)
    {
        ...
        if (accessTokenIssued && idTokenIssued && refreshTokenIssued)
        {
            expectedTags.Add(TelemetryConstants.TagConstants.TokenType, $"{IdentityServerConstants.TokenTypes.AccessToken} {IdentityServerConstants.TokenTypes.IdentityToken} {IdentityServerConstants.PersistedGrantTypes.RefreshToken}");
        }
        else
        {
            expectedTags.Add(TelemetryConstants.TagConstants.TokenType, "");
        }
        if (error != null) expectedTags.Add(TelemetryConstants.TagConstants.Error, error);


        _subject.CountTokenIssued(client, grantType, accessTokenIssued, idTokenIssued, refreshTokenIssued, error);
        ...
        string error)
    {
        ...
        _subject.CountTokenIssued(client, grantType, true, true, true, error);
